...
class AddressChangeTest extends TestCase
{
    ...
    {
        $user = User::factory()->create(['email_verified_at' => now()]);
        $response = $this->actingAs($user)->post('/address/update', ['postcode' => '123-4567', 'address' => '東京都渋谷区1-2-3', 'building' => 'テストビル101']);
        $response->assertStatus(302);
        $this->assertDatabaseHas('profiles', ['user_id' => $user->id, 'postcode' => '123-4567']);
    }
    ...
    public function test_user_can_update_address_during_purchase()
    {
        $user = User::factory()->create(['email_verified_at' => now()]);
        $item = Item::factory()->create();
        $response = $this->actingAs($user)->post("/purchase/address/{$item->id}", ['postcode' => '987-6543', 'address' => '大阪府大阪市', 'building' => '更新ビル']);
        $response->assertStatus(302);
    }
}
